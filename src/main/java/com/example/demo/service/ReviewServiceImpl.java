...
public class ReviewServiceImpl implements ReviewService {
    ...
    /* 
    @Override
    public List<ReviewResponse> findByPlaceId(Integer placeId, int page, int pageSize) {
        if (!placeRepo.existsById(placeId)) {
            throw new EntityNotFoundException("Place not found: " + placeId);
        }

        return repository.findByPlaceIdWithPagination(placeId, page, pageSize).stream()
    ...
    */
    ...
    @Override
    public List<ReviewResponse> findByPlaceIdWithPagination (Integer placeId, int page, int pageSize) {
        return repository.findByPlaceIdWithPagination(placeId, page, pageSize).stream()
                .map(ReviewMapper::toResponse)
                .toList();
    }
